...
\section{RESULTS AND DISCUSSIONS} \label{sec:results}
    ...
    \subsection{Impact of Spoofed Position}
        ...
        \begin{itemize}
            \item \textbf{Carrier-to-noise density (C/N):} The plot of the signal quality remains unchanged compared to the base case, this because the signals are just replayed with a different timing to the victim.
    ...
    \subsection{Interference Effects}
        ...
        \noindent
        To assess the impact of interference on smartphone GNSS performance, we conducted a controlled static experiment where the device was wrapped in two layers of aluminum foil and placed between two active smartphones generating Wi-Fi and Bluetooth transmissions. This setup aimed to replicate real-world RF interference and physical signal attenuation without transmitting in GNSS bands directly.

        The effects were immediately evident in the recorded data. Pseudorange measurements exhibited delayed acquisition and clock discontinuities (Figure~\ref{fig:interference_pr}), indicating disrupted signal tracking at the start and end of the session. Additionally, the consistency between reported and calculated pseudorange rates degraded substantially, with some satellites showing large and persistent biases—suggesting timing instability under noisy conditions.

        Signal quality, as measured by carrier-to-noise ratio (C/N), dropped sharply during the initial phase, often falling below 25 dB-Hz. This reduction, visible in Figure~\ref{fig:interference_cno}, confirms that the aluminum shielding and nearby devices significantly weakened GNSS signal reception.

        Perhaps the most critical observation lies in the WLS-derived position estimates. Despite the phone remaining stationary, the position solution drifted more than 100 meters across the session. As shown in Figure~\ref{fig:interference_pos}, the horizontal accuracy degraded severely, and HDOP remained elevated due to reduced satellite visibility.
        ...
        \noindent
        These results show that even low-grade interference from nearby consumer electronics and basic shielding can induce measurable degradation in GNSS performance. Such vulnerability may go unnoticed by standard signal quality checks, highlighting the need for robust cross-layer detection mechanisms in mobile GNSS receivers.
